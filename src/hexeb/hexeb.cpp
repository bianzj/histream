...
bool Hexeb::setup(AppSetting &appsetting, std::shared_ptr<HexebIO> &modelio){
...
bool Hexeb::upload(std::shared_ptr<FileIO> &fileio, std::shared_ptr<HexebIO> &modelio){
    ...
    // hex: 场景构建前开启异质性体元计算 (写入 VoxelIO::voxelHexs / VoxelLink::hexId)
    m_pScene->hexVoxelEnabled = true;
...
bool Hexeb::uploadSetting(std::shared_ptr<FileIO> &fileio, std::shared_ptr<HexebIO> &modelio) {
...
bool Hexeb::updateSetting(std::shared_ptr<HexebIO> &modelio){
...
bool  Hexeb::uploadMeteo(std::shared_ptr<FileIO> &fileio, std::shared_ptr<HexebIO> &modelio){
...
bool  Hexeb::updateMeteo(std::shared_ptr<HexebIO> &modelio, int knode){
...
bool Hexeb::uploadDefined(std::shared_ptr<FileIO> &fileio, std::shared_ptr<HexebIO> &modelio)
{
...
bool Hexeb::create(std::shared_ptr<HexebIO> &modelio) {
...
bool Hexeb::run(std::shared_ptr<HexebIO> &modelio, std::shared_ptr<FileIO> &fileio) {
...
// for(int knode = 72; knode < 75;knode = knode + 1) {
    ...
    // for(int knode = 0; knode < 144;knode = knode + 1) {
        ...
        // The shadow/transmittance stage is the first energy-balance stage,
        // so the current time's solar direction must already be on the GPU.
...
bool Hexeb::destroy(std::shared_ptr<HexebIO> & modelio){
...
void Hexeb::output(std::shared_ptr<HexebIO> &modelio, std::shared_ptr<FileIO> &fileio, int knode, int kangle) {
...
void Hexeb::outputTxt(std::shared_ptr<HexebIO>& modelio, std::shared_ptr<FileIO>& fileio, int kangle)
{
    VkBufferUsageFlags usage{VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT};
    const int width = modelio->imageSize.x;
    const int height = modelio->imageSize.y;
...
void Hexeb::outputVoxel(std::shared_ptr<HexebIO> &modelio, std::shared_ptr<FileIO> &fileio) {
    (void)fileio;
...
bool Hexeb::uploadAero(std::shared_ptr<FileIO> &fileio, std::shared_ptr<HexebIO> &modelio) {
