...
bool Command::create(std::shared_ptr<HexebIO> &modelio){
    // for the firest init
...
bool Command::runEB(std::shared_ptr<HexebIO> &modelio){
...
bool Command::runRT(std::shared_ptr<HexebIO> &modelio){
...
//bool Command::run(std::shared_ptr<HexebIO> &modelio){
//
//    modelio->m_currentSemaphore = 1;
//
//    glm::ivec3 voxelSize1D = glm::ivec3((modelio->n_voxel + (GROUP_SIZEX - 1)) / GROUP_SIZEX, 1, 1);
...
void Command::submit(std::shared_ptr<HexebIO> &modelio, VoxelEBStage stage, glm::ivec3 dispatchSize,
...
//    vkBeginCommandBuffer(cmdBuf, &beginInfo);
    ...
    recordHexebCommandBuffer(cmdBuf, descSet, pipelineLayout, pipeline, setting );
...
//void Command::submit(std::shared_ptr<HexebIO> &modelio,glm::ivec3 dispatchSize,
...
void Command::waitFence(std::shared_ptr<HexebIO> &modelio)
{
    vkWaitForFences(modelio->m_device, 1, &(modelio->m_fence), VK_TRUE, UINT64_MAX);
    vkResetFences(modelio->m_device, 1,  &(modelio->m_fence));
}
...
void Command::destroy(std::shared_ptr<HexebIO> &modelio) {
