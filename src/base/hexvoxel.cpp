...
bool loadObj(const std::string& path, float voxelSize, HexScene& out, std::string* err)
{
    ...
    std::map<std::string, std::uint32_t> objIndex; // 名称 -> 对象编号
    ...
    while (std::getline(f, line)) {
        ...
        if (line.compare(0, 2, "v ") == 0) {
            ...
            while (n < 32) {
                ...
                if (end == rest || v <= 0) break;
                idx[n++] = static_cast<std::uint32_t>(v - 1);
                rest = end;
            }
            if (n >= 3) {
                if (!haveObject) {
                    curObject = "default";
                    haveObject = true;
                    objIndex[curObject] = static_cast<std::uint32_t>(faces.size());
                    objTriStart.push_back(static_cast<std::uint32_t>(faces.size()));
                }
                // n-gon 扇形剖分 (OBJ 常见为 4 顶点面)
                for (int t = 1; t + 1 < n; ++t) {
                    faces.push_back({idx[0], idx[t], idx[t + 1], objIndex[curObject]});
                }
            }
        } else if (line.compare(0, 2, "o ") == 0 || line.compare(0, 2, "g ") == 0) {
            std::string name = line.substr(2);
            if (!name.empty() && name[0] == ' ') name = name.substr(1);
            if (name.empty()) name = "object" + std::to_string(objIndex.size() + 1);
            if (objIndex.find(name) == objIndex.end()) {
                objIndex[name] = static_cast<std::uint32_t>(faces.size());
                objTriStart.push_back(static_cast<std::uint32_t>(faces.size()));
            }
            curObject = name;
            haveObject = true;
    ...
    for (std::size_t o = 0; o < objTriStart.size(); ++o) {
        std::string name;
        for (const auto& kv : objIndex) {
            if (kv.second == objTriStart[o]) { name = kv.first; break; }
        }
        out.objects[o].name = name;
            ...
            for (int y = 0; y < gy; ++y) {
                for (int z = 0; z < gz; ++z) {
                    ...
                    for (int axis = 0; axis < 3; ++axis) {
                        const float cover = projectionCoverageAxis(c, x, y, z, axis, n);
                        float ci = 1.0f;
