...
bool Command::create(std::shared_ptr<HexrtIO> &modelio){
    // for the firest init
...
bool Command::runRT(std::shared_ptr<HexrtIO> &modelio){
...
void Command::submit(std::shared_ptr<HexrtIO> &modelio, VoxelRTStage stage, glm::ivec3 dispatchSize,
                     const std::optional<std::vector<VkSemaphore>> &inSemaphores, const std::optional<VkSemaphore> &outSemaphore)
{
    ...
    recordHexrtCommandBuffer(cmdBuf, descSet, pipelineLayout, pipeline, setting );
...
void Command::waitFence(std::shared_ptr<HexrtIO> &modelio)
{
    vkWaitForFences(modelio->m_device, 1, &(modelio->m_fence), VK_TRUE, UINT64_MAX);
    vkResetFences(modelio->m_device, 1,  &(modelio->m_fence));
}
...
void Command::destroy(std::shared_ptr<HexrtIO> &modelio) {
