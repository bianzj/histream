...
bool Buffer::createBuffer(std::shared_ptr<HexrtIO> &voxellstio){
...
//    }
    ...
    if (voxelio->nanoHandle.size() > 0)
    {
        ...
        // hex: 异质性体元参数 (Ax, Ay, Az, rho); 至少保留一个空条目,
        // 使着色器中 voxelHexs[hexId] 的引用始终有效
        if (voxelio->voxelHexs.empty()) {
            voxelio->voxelHexs.emplace_back();
        }
        VkCommandBuffer cmdBufVoxelHex = cmdGen.createCommandBuffer();
        voxelio->m_pVoxelHexBuffer = std::make_shared<nvvk::Buffer>(m_pAlloc->createBuffer(cmdBufVoxelHex, voxelio->voxelHexs, usage_));
...
void Buffer::destroy(std::shared_ptr<HexrtIO> &voxellstio){
